...
public class AdvancedEmbeddingScenarios
{
    ...
    public async Task TonalShift_CMajor_vs_GMajor_StructuralSimilarity()
    {
        ...
        TestContext.WriteLine(
            $"Structural Similarity (C vs G): Expected > 0.6, Actual={sim:P2} (Chords of the same type share functional and structural properties regardless of root)");
        // They should have VERY high structural similarity because they share:
        // 1. Cardinality
        // 2. ICV
        // 3. Functional properties (Triad consonance/brightness)
        Assert.That(sim, Is.GreaterThan(0.6),
            "Tonal shifts of the same chord type should be structurally similar (similarity > 0.6).");
    }
    ...
    public async Task SymbolicImpact_C7_with_HendrixTag()
    {
        ...
        var docHendrix = CreateDummyDocument("x323xx", "C7", [0, 4, 10], "000100", ["hendrix"]);
        // Act
        ...
        Assert.Multiple(() =>
        {
            Assert.That(sim, Is.EqualTo(0.0),
                "Untagged vs Tagged should have zero symbolic similarity if no tags match.");
            Assert.That(symbolicHendrix.Any(v => v > 0.1), Is.True,
                "Symbolic vector should have energy when tags are present.");
        });
    }
    ...
    public async Task SymbolicImpact_JamesBondChord_Tag()
    {
        ...
        var docBond = CreateDummyDocument("x21000", "Em(maj7)", [4, 7, 11, 3], "100111", ["james-bond-chord"]);
        // Act
        var vNormal = await _generator.GenerateEmbeddingAsync(docNormal);
        ...
        Assert.Multiple(() =>
        {
            Assert.That(symbolicBond[11], Is.EqualTo(1.0),
                "James Bond chord tag should set the 'Iconic' bit (Index 11).");
    ...
    public async Task MorphologyShift_Open_vs_Barre()
    {
        ...
        var docBarre = CreateDummyDocument("x35553", "C", [0, 4, 7], "001110", minFret: 3, maxFret: 5, stretch: 2,
            barre: true);
        // Act
        ...
        Assert.That(sim, Is.LessThan(0.8),
    ...
    public async Task SymbolicImpact_MultiSourceYAML_Tags()
    {
        ...
        var doc = CreateDummyDocument("x32010", "C", [0, 4, 7], "100111", ["sweep-picking", "smooth-voice-leading"]);
        // Act
        ...
        // Assert
        TestContext.WriteLine(
            $"Symbolic Vector for complex tags: [{string.Join(", ", symbolic.Select(x => x.ToString("F1")))}]");
