...
public class ExtendedTextureFeatureTests
{
    ...
    public async Task SpectralEntropy_CleanTriad_HigherThan_SingleNote()
    {
        ...
        var singleDoc = CreateTestDocument([60]);
        // Major Triad (Some peaks k=3,4,5) -> Lower Entropy -> Result > 0.0
        var triadDoc = CreateTestDocument([60, 64, 67]); // C, E, G
        // Act
        ...
        TestContext.WriteLine(
            $"Comparison: Expected {triadEntropy:F4} > {singleEntropy:F4} + 0.1, Actual Result={triadEntropy > singleEntropy + 0.1} (Triads have more organized spectral peaks than a flat impulse spectrum)");
    ...
    public async Task SalientChroma_Doubling_ChangesSpectrum()
    {
        ...
        var docA = CreateTestDocument([48, 52, 55]); // C, E, G
        // Case B: C Major with doubled Root - {C, C, C, E, G} -> Chroma [3, 0, 0, 0, 1, 0, 0, 1, ...]
        var docB = CreateTestDocument([48, 60, 72, 52, 55]); // C, C, C, E, G
        // Act
    ...
    public async Task HarmonicInertia_StableChord_HighValue()
    {
        ...
        var stableDoc = CreateTestDocument([48, 52, 55], consonance: 0.9);
        // Act
    ...
    public async Task ResolutionPressure_TenseChord_HighValue()
    {
        ...
        var tenseDoc = CreateTestDocument([48, 54, 60], consonance: 0.2);
        // Act
    ...
    public async Task DoublingRatio_DoubledNotes_PositiveValue()
    {
        ...
        var doc = CreateTestDocument([48, 52, 55, 60], [0, 4, 7, 0]);
        // Act
    ...
    public async Task RootDoubled_WhenRootAppearsMultipleTimes_ReturnsOne()
    {
        ...
        var doc = CreateTestDocument([48, 52, 55, 60], [0, 4, 7, 0], 0);
        // Act
        var embedding = await _generator.GenerateEmbeddingAsync(doc);
        var rootDoubled = embedding[EmbeddingSchema.TexturalRootDoubled];
        // Assert
        TestContext.WriteLine(
            $"Root: 0, Pitch Classes: [0, 4, 7, 0], Root Doubled Flag: Expected=1.0, Actual={rootDoubled} (Root C=0 is present twice in the voicing)");
        Assert.That(rootDoubled, Is.EqualTo(1.0),
            "Root doubled flag should be 1.0 when the root pitch class appears more than once.");
    }
    ...
    public async Task BassMelodySpan_WideVoicing_HighValue()
    {
        ...
        var wideDoc = CreateTestDocument([40, 52, 64]);
        // Act
    ...
    public async Task OpenPosition_SpanGreaterThanOctave_ReturnsOne()
    {
        // Arrange
        // Span > 12 semitones
        var openDoc = CreateTestDocument([40, 47, 55, 64]);
        // Act
        ...
        // Assert
        TestContext.WriteLine(
            $"Open Voicing Span: {openDoc.MidiNotes.Max() - openDoc.MidiNotes.Min()} semitones, Open Position Flag: {openPosition}");
    ...
    public async Task OpenPosition_SpanWithinOctave_ReturnsZero()
    {
        // Arrange
        // Span <= 12 semitones
        var closedDoc = CreateTestDocument([60, 64, 67]);
        // Act
        var embedding = await _generator.GenerateEmbeddingAsync(closedDoc);
        var openPosition = embedding[EmbeddingSchema.ExtendedOpenPosition];
        // Assert
        TestContext.WriteLine(
            $"Closed Voicing Span: {closedDoc.MidiNotes.Max() - closedDoc.MidiNotes.Min()} semitones, Open Position Flag: {openPosition}");
    ...
    public async Task InnerVoiceDensity_MidRangeVoicing_HighValue()
    {
        ...
        var midDoc = CreateTestDocument([55, 60, 64, 67]);
        // Act
    ...
    public async Task LocalClustering_ClusterVoicing_HighValue()
    {
        ...
        var clusterDoc = CreateTestDocument([60, 62, 64]);
        // Act
    ...
    public async Task RoughnessProxy_LowCluster_HighValue()
    {
        ...
        var lowClusterDoc = CreateTestDocument([40, 41, 42]);
        // Act
