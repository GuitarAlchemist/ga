...
public class ScaleConfigTests
{
    ...
    public void Test1()
    {
        var ids = PitchClassSetId.Items.Where(id => id.IsScale && !id.ToPitchClassSet().IsModal).Select(id => id.Value)
            .ToImmutableList();
        ...
        {
            if (!scaleByPcs.ContainsKey(scale.PitchClassSet))
            {
                scaleByPcs[scale.PitchClassSet] = [];
            }
