...
public class SemanticSearchTests
{
    ...
    private async Task<IEnumerable<VoicingSearchResult>> SearchAndLog(string query)
    {
        ...
        var results =
            await SearchService.SearchAsync(query, t => Task.FromResult(SemanticSearchTestFixture.MockEmbed(t)));
        if (results.Any())
        {
            var top = results.First();
            TestContext.Progress.WriteLine(
                $"[RESULT] Top: \"{top.Document.ChordName}\" | Score: {top.Score:F4} | Tags: [{string.Join(", ", top.Document.SemanticTags)}]");
        }
        else
        {
    ...
    public async Task Search_BarreChords_ReturnsBarreVoicings()
    {
        ...
        Assert.That(results, Is.Not.Empty);
        var top = results.First();
        Assert.That(top.Document.Position, Is.EqualTo("Barre"),
            $"Result '{top.Document.ChordName}' should be a barre chord");
    ...
    public async Task Search_CommonChords_ReturnsTriads()
    {
        ...
        }; // C Major in test set might not be tagged common? Let's check G/D/Em
    ...
    public async Task Search_HendrixChord_Returns7Shar9()
    {
        ...
        {
            TestContext.Progress.WriteLine($" - {r.Document.ChordName}: {r.Score}");
        }
    ...
    public async Task Search_OpenChords_ReturnsOpenPosition()
    {
        ...
        Assert.That(result.Document.Difficulty == "Beginner", Is.True,
            $"Result '{result.Document.Id}' should be beginner difficulty");
    }
    ...
    public async Task Search_JazzyChords_ReturnsComplexHarmony()
    {
        ...
        if (result.Document.ChordName == "G Major")
        {
            ...
            Assert.Fail(
                $"Top: {result.Document.ChordName} ({result.Score}). C13: {c13?.Document.ChordName} ({c13?.Score})");
        }
