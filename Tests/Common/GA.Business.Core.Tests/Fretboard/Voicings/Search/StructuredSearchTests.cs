...
public class StructuredSearchTests
{
    ...
    public async Task Search_FingerCount_ReturnsThreeFingers()
    {
        ...
        var cMajor =
            results.FirstOrDefault(r =>
                r.Document.ChordName ==
                "C Major 7"); // x32010 is usually Cmaj7 shell? No, test says Cmaj7 diagram is dummy "x32010"
    ...
    public async Task Search_IsNaturallyOccurring_ExcludesChromatic()
    {
        ...
        var results = await SearchService.SearchAsync("out",
            ...
            10,
            filters);
        Assert.That(results.Any(r => r.Document.Id == "test-Chromatic"), Is.False,
            "Should exclude non-diatonic chords");
    }
