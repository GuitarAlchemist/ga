...
public static class MusicalAssertions
{
        ...
        double warningThreshold = 3.5)
    {
        ...
        else if (cost.TotalCost > warningThreshold)
        {
            TestContext.WriteLine(
                $"[WARNING] Shape is difficult but acceptable (Cost: {cost.TotalCost:F2}). Breakdown: {breakdownStr}");
        }
        else
        {
    ...
    public static void AreHarmonicallyEquivalent(List<FretboardPosition> expected, List<FretboardPosition> actual)
    {
        ...
        for (var i = 0; i < expectedPitches.Count; i++)
        {
    ...
    /// </summary>
    public static void IsSmoothSequence(List<List<FretboardPosition>> sequence, PhysicalCostService costService,
        double maxAvgTransition = 6.0)
    {
        if (sequence.Count < 2)
        {
            return;
        ...
        var avg = totalTrans / (sequence.Count - 1);
