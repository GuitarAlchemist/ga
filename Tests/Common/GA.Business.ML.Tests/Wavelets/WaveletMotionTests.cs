...
public class WaveletMotionTests
{
    ...
    public void Setup()
    {
        _signalService = new();
        _waveletService = new();
        _embeddingService = new(_signalService, _waveletService);
    }
    ...
    private ChordVoicingRagDocument MockVoicing(int[] pcs, double consonance = 0.5, double entropy = 0.5)
    {
        ...
        {
            emb[EmbeddingSchema.SpectralEntropy] = (float)entropy;
        }
        ...
        return new()
        {
            ...
            Embedding = emb,
            PitchClasses = pcs,
            MidiNotes = [.. pcs.Select(p => 60 + p)], // Assume Middle C octave

            // Required props
            RootPitchClass = pcs.Length > 0 ? pcs[0] : 0,
            SemanticTags = [], PossibleKeys = [], YamlAnalysis = "{}", PitchClassSet = string.Join(",", pcs),
            IntervalClassVector = "000000", AnalysisEngine = "Test", AnalysisVersion = "1.0", Jobs = [],
            TuningId = "Standard", PitchClassSetId = "0", Diagram = "", BarreRequired = false, HandStretch = 0,
            MinFret = 0, MaxFret = 0
        };
    }
    ...
    public void Test_RisingTension_SignalDetection()
    {
        ...
        {
            MockVoicing([0, 4, 7], 1.0), // C Major (Stable)
            MockVoicing([0, 4, 7, 10], 0.7), // C7
            MockVoicing([0, 4, 7, 10, 1], 0.4), // C7b9
            MockVoicing([0, 3, 6, 9], 0.2) // Cdim7 (Unstable)
        };
    ...
    public void Test_StaticHarmony_ZeroVelocity()
    {
        ...
        var cMaj = new[] { 0, 4, 7 };
        var progression = Enumerable.Repeat(MockVoicing(cMaj, 1.0), 4).ToList();
        ...
        {
            progression[i] = progression[i] with
            {
                Embedding = generator.GenerateEmbeddingAsync(progression[i]).GetAwaiter().GetResult()
            };
        }
    ...
    public void Test_Modulation_SpikeInVelocity()
    {
        ...
        var progression = new List<ChordVoicingRagDocument>
        {
            MockVoicing(cMaj, 1.0),
            MockVoicing(cMaj, 1.0),
            MockVoicing(fsMaj, 1.0), // Jump here
            MockVoicing(fsMaj, 1.0)
        };
        ...
        var decomp = _waveletService.Decompose(signals.Velocity);
        var detailEnergy = decomp.DetailCoefficients.Sum(level => level.Sum(x => x * x));
