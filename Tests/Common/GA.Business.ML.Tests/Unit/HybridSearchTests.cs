...
public class HybridSearchTests
{
    ...
    public void Setup()
    {
        _mockIndex = new Mock<IVectorIndex>();
        _retrievalService = new SpectralRetrievalService(_mockIndex.Object);
    }
    ...
    public void Search_WithQualityFilter_ReturnsOnlyMatchingDocuments()
    {
        ...
        var queryVector = new double[EmbeddingSchema.TotalDimension];

        // Act: Filter by "Minor" quality tag
        var results = _retrievalService.Search(queryVector, topK: 10, quality: "Minor").ToList();
    ...
    public void Search_WithMultipleFilters_FiltersCorrectly()
    {
        ...
        var docs = new List<VoicingDocument>
        {
            ...
            CreateDoc("2", "C Major 7 Tertian", "x-x-x", "Major", "7", "Tertian"),
    ...
    public void Search_WithNoFilters_ReturnsAllDocuments()
    {
        ...
        var results = _retrievalService.Search(queryVector, topK: 10).ToList();
    ...
    public void Search_WithNoteCountFilter_FiltersCorrectly()
    {
        ...
        var docs = new List<VoicingDocument> { doc3Notes, doc4Notes };
        _mockIndex.Setup(i => i.Documents).Returns(docs);

        var queryVector = new double[EmbeddingSchema.TotalDimension];

        // Act: Filter to 4-note voicings only
        var results = _retrievalService.Search(queryVector, topK: 10, noteCount: 4).ToList();
    ...
    private VoicingDocument CreateDoc(
        string id, 
        string name, 
        string diagram, 
        string tag, 
        ...
        int[]? midiNotes = null)
    {
        ...
        if (extension != null) tags.Add(extension);
