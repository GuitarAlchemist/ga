...
[UsedImplicitly]
public class PitchClassSetSyncService(ILogger<PitchClassSetSyncService> logger, MongoDbService mongoDb)
    : ISyncService<PitchClassSetDocument>
{
    public async Task<bool> SyncAsync()
    {
        try
        {
            var documents = PitchClassSet.Items.Select(pitchClassSet => new PitchClassSetDocument
            {
                ...
                PitchClasses = [.. pitchClassSet.Select(pc => pc.Value)],
                Notes = [.. pitchClassSet.Notes.Select(n => n.ToString())],
