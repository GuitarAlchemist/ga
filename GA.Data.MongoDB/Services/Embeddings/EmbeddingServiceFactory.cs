    ...
    public IEmbeddingService CreateService()
    {
        return settings.Value.ServiceType switch
        {
            ...
            EmbeddingServiceType.AzureOpenAi => new AzureOpenAiEmbeddingService(
                ...
                settings.Value.ApiKey,
                settings.Value.DeploymentName ?? throw new ArgumentNullException(nameof(EmbeddingServiceSettings.DeploymentName))),
            
            EmbeddingServiceType.OnnxLocal => new OnnxEmbeddingService(
                settings.Value.ModelPath ?? throw new ArgumentNullException(nameof(EmbeddingServiceSettings.ModelPath))),
