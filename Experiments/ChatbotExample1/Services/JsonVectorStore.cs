...
public class JsonVectorStore(string basePath) : IVectorStore
{
        ...
        where TKey : notnull
    {
        ...
        public Task DeleteAsync(TKey key, DeleteRecordOptions? options = null,
        ...
        public Task DeleteBatchAsync(IEnumerable<TKey> keys, DeleteRecordOptions? options = null,
            CancellationToken cancellationToken = default)
        {
        ...
        public async IAsyncEnumerable<TKey> UpsertBatchAsync(IEnumerable<TRecord> records,
            UpsertRecordOptions? options = null, [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
        ...
        public Task<VectorSearchResults<TRecord>> VectorizedSearchAsync<TVector>(TVector vector,
            VectorSearchOptions? options = null, CancellationToken cancellationToken = default)
        {
            if (vector is not ReadOnlyMemory<float> floatVector)
            {
                throw new NotSupportedException(
                    $"The provided vector type {vector!.GetType().FullName} is not supported.");
            }
            ...
            foreach (var clause in options?.Filter?.FilterClauses ?? [])
            {
                if (clause is EqualToFilterClause equalClause)
                {
                    ...
                    filteredRecords =
                        filteredRecords.Where(record => propertyInfo!.GetValue(record)!.Equals(equalClause.Value));
                }
                else
                {
                    throw new NotSupportedException(
                        $"The provided filter clause type {clause.GetType().FullName} is not supported.");
                }
            }
