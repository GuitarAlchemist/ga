...
- [x] `GET /api/contextual-chords/voicings/Bm/comfort` returns voicings sorted by stretch ascending.
- [x] Full barre voicings (4+ strings on the same fret) are excluded by default (`excludeBarre=true`).
- [x] Passing `excludeBarre=false` returns all voicings including barre chords, still sorted by stretch.
- [x] Open strings (fret 0) are excluded from the stretch calculation.
- [x] `VoicingFilterService.Filter()` returns `Result<IReadOnlyList<Voicing>, VoicingFilterError>` — no `ArgumentException` thrown.
- [x] All existing `VoicingFilterService` tests pass after the ROP fix (signatures updated).
- [x] Frontend: clicking a chord in `VexTabViewer` fetches and renders the lowest-stretch voicing.
- [x] `dotnet build AllProjects.slnx -c Debug` passes with zero warnings in touched files.
- [x] `npm run build` and `npm run lint` pass in `ReactComponents/ga-react-components`.
- [x] Unit tests added in `Tests/Apps/GaApi.Tests/Services/VoicingComfortServiceTests.cs` covering:
  - [x] Stretch calculation excludes open strings.
  - [x] Barre detection flags voicings with 4+ strings on the same lowest fret.
  - [x] Results are sorted by stretch ascending.
  - [x] `excludeBarre=false` includes barre chords.
...
- [x] Fix `VoicingFilterService.Filter()` to return `Result<IReadOnlyList<Voicing>, VoicingFilterError>` (ROP).
  - [x] Add `VoicingFilterError` enum.
  - [x] Update `ContextualChordsController` to convert `VoicingFilterError` to HTTP responses at the boundary.
  - [x] Update `VoicingFilterServiceTests.cs` for the new signature.
- [x] Create `VoicingComfortService.cs` in `Apps/ga-server/GaApi/Services/`.
  - [x] Implement `ComputeStretch(Voicing)` — excludes open and muted strings.
  - [x] Implement `IsFullBarre(Voicing)` — 4+ strings share the same lowest fret.
  - [x] Implement `GetComfortRankedAsync(chordName, excludeFullBarre, ct)` returning `Result<T, VoicingError>`.
- [x] Register `VoicingComfortService` in GaApi DI container.
- [x] Add `GET voicings/{chordName}/comfort` action to `ContextualChordsController`.
- [x] Create `useComfortVoicing.ts` hook in `ReactComponents/ga-react-components/src/hooks/`.
- [x] Wire `VexTabViewer` `onChordClick` to call `useComfortVoicing` and render the result.
- [x] Write unit tests in `Tests/Apps/GaApi.Tests/Services/VoicingComfortServiceTests.cs`.
- [x] Run `dotnet build AllProjects.slnx -c Debug`, `dotnet test AllProjects.slnx`, `npm run build`, `npm run lint` — all must pass.
