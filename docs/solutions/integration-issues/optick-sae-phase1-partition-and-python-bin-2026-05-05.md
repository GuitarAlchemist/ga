...
- **Producer/consumer schema contract test.** Add a unit test in `crates/ix-optick-sae` that loads the canonical artifact JSON (vendored as a test fixture, refreshed by CI from the GA repo) and asserts `PHASE1_PARTITIONS == artifact["partitions_used"]` and `compact_training_dim == sum(partition_widths)`. This is the test that would have caught both the original PR #82 miss and this recurrence on first run, before any smoke.
- **The "ROOT was missed twice" invariant.** Add a stronger property: any partition list claiming to represent OPTIC-K similarity features must include `ROOT` (slots 228..240) as long as `EmbeddingSchema.Version >= "OPTIC-K-v1.8"`. Encode it as `assert "ROOT" in PHASE1_PARTITIONS, "ROOT required since v1.8 (T-invariance)"` and mirror it in Rust. The lesson: similarity-feature lists are a one-way door (re-indexing cost is high) and deserve a hard invariant, not a code comment.
...
- **End-to-end smoke is the gate, not unit tests.** For cross-language pipelines (Rust spawns Python, .NET calls F#, frontend calls API), green unit tests on either side prove nothing about the boundary. CI must run the full producer → subprocess → artifact → consumer-validation loop on each target OS (Windows + Linux) before merge. "It compiled and tests passed" is not success on this risk class.
- **Cross-repo PRs trigger multi-LLM review.** Per the recent chatbot-migration evidence (9 real bugs caught), any PR crossing a producer/consumer schema boundary — SAE artifacts, OPTIC-K dims, MCP tool contracts — gets `octo-code-reviewer` + `octo-security-auditor` in parallel. Local tests didn't catch ROOT being missed twice; reviewers reading the canonical baseline alongside the diff would have.
- **CI status before declaring shipped.** `gh pr view --json statusCheckRollup` after every push. Local Windows green != CI green, especially when CI runs Linux and the bug is Windows-only (or vice versa). The admin override on ix #29 was justified only because the failing nightlies were demonstrably pre-existing on `main`; default posture is wait for green.
