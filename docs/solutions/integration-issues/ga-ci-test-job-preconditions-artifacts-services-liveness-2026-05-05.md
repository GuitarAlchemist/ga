...
### Related PRs
- [`GuitarAlchemist/ga#122`](https://github.com/GuitarAlchemist/ga/pull/122) — the PR being documented. Adds `actions/download-artifact@v4` for `--no-build` test jobs, starts `GaApi` via `Start-Process -NoNewWindow dotnet GaApi.dll` with stdout/stderr capture, polls `/api/chatbot/status` with the 401-blind probe, uploads `gaapi-logs` artifact on timeout.
