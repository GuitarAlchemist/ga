...
- [ ] **(critical for chatbot grounding)** `GpuVoicingSearchStrategy` init pipeline does not transfer embeddings into the search structure. After `InitializeEmbeddingsAsync` completes with "Voicing search index initialized successfully with 667125 voicings in 16.4s total", the search stats report `667125 voicings, 0 MB memory, 0ms avg search time` and every query throws `ArgumentException: Query embedding dimension mismatch. Expected 0, got 768` at `GpuVoicingSearchStrategy.CalculateCosineSimilarity`. The 667k embeddings DO load from the binary cache (`voicing_embeddings_667125.bin`, 10s) but the InitializeEmbeddingsAsync inner path doesn't populate the GPU/CPU search arrays with them. Surfaced 2026-04-18 after upgrading `SemanticKnowledgeSource` to log errors instead of warnings. Next session: audit `EnhancedVoicingSearchService.InitializeEmbeddingsAsync` → `GpuVoicingSearchStrategy` to find where the 768-dim Ollama embeddings get dropped vs. assigned an expected dimension of 0.
