...
{{
  ...
  ""negative_factors"": [""factor1"", ""factor2""]
            ...
            if (jsonMatch.Success)
            {
                ...
                if (root.TryGetProperty("positive_factors", out var positive))
                {
                    foreach (var factor in positive.EnumerateArray())
                    {
                        var factorStr = factor.GetString();
                        if (factorStr != null)
                        {
                            evaluation.PositiveFactors.Add(factorStr);
                        }
                    }
                }

                if (root.TryGetProperty("negative_factors", out var negative))
                {
                    foreach (var factor in negative.EnumerateArray())
                    {
                        var factorStr = factor.GetString();
                        if (factorStr != null)
                        {
                            evaluation.NegativeFactors.Add(factorStr);
                        }
                    }
                }
            }
