    ...
    where T : IRangeValueObject<T>
{
    ...
    IEnumerator<T> IEnumerable<T>.GetEnumerator() => new Enumerator(_items, _start, Count);

    IEnumerator IEnumerable.GetEnumerator() => new Enumerator(_items, _start, Count);
    ...
    public static ValueObjectCollection<T> Create(int start, int count)
    {
        ...
        if (start >= min && start + count - 1 <= max)
        {
            ...
            return new(ValueObjectCache<T>.AllItems, offset, count);
        }
    ...
    public Enumerator GetEnumerator() => new(_items, _start, Count);
