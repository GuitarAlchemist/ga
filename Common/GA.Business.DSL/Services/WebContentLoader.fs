...
module WebContentLoader =
    ...
    do httpClient.DefaultRequestHeaders.Add("User-Agent", "GuitarAlchemist/1.0 (Educational Music Software)")
    ...
    let isSupportedContentType (contentType: string) =
        ...
        supportedTypes
        |> List.exists (fun t -> contentType.Contains(t, StringComparison.OrdinalIgnoreCase))
    ...
    let loadFromSongsterr (searchCriteria: SearchCriteria option) : Task<Result<string, string>> =
        task {
            let searchQuery =
                match searchCriteria with
                | Some criteria -> [ criteria.Artist; criteria.Title ] |> List.choose id |> String.concat " "
                | None -> "guitar tabs"
    ...
    let loadFromMuseScore (searchCriteria: SearchCriteria option) : Task<Result<string, string>> =
        task {
            let searchQuery =
                match searchCriteria with
                | Some criteria -> [ criteria.Title; criteria.Genre ] |> List.choose id |> String.concat " "
                | None -> "sheet music"
    ...
    let loadFromIMSLP (searchCriteria: SearchCriteria option) : Task<Result<string, string>> =
        task {
            let searchQuery =
                match searchCriteria with
                | Some criteria -> [ criteria.Title; criteria.Artist ] |> List.choose id |> String.concat " "
