...
module ContentValidator =
    ...
    let isTrustedDomain (url: string) =
        try
            ...
            trustedDomains
            |> List.exists (fun domain -> host = domain || host.EndsWith($".{domain}"))
        with _ ->
            false
    ...
    let validateAsciiTab (content: string) =
        ...
        let hasTabLines =
            ...
            |> Array.exists (fun line ->
                Regex.IsMatch(line, @"^[eEbBgGdDaA]\|.*\|")
                || // Standard tab notation
                Regex.IsMatch(line, @"^[1-6]\|.*\|")
                || // Numbered strings
    ...
    let validateGuitarProFormat (bytes: byte[]) =
        ...
        let header =
            System.Text.Encoding.ASCII.GetString(bytes.[0 .. Math.Min(19, bytes.Length - 1)])
    ...
    let validateMusicXmlFormat (content: string) =
        if
            ...
            && (content.Contains("<score-partwise") || content.Contains("<score-timewise"))
        then
            Ok content
        else
    ...
    let validateContentSize (contentType: string) (size: int64) =
        let maxSize =
            maxFileSizes |> Map.tryFind contentType |> Option.defaultValue (1024L * 1024L) // Default 1MB
    ...
    let validateLicense (licenseType: LicenseType option) =
        ...
        | Some PublicDomain
        | Some CreativeCommons
        | Some Free
        | Some EducationalUse -> Ok "Content has appropriate license for educational use"
        | None -> Ok "No license specified - proceed with caution"
    ...
    let validateSearchCriteria (criteria: SearchCriteria) =
        ...
        let hasSuspiciousContent =
            suspiciousPatterns
            |> List.exists (fun pattern -> Regex.IsMatch(searchText, pattern, RegexOptions.IgnoreCase))
