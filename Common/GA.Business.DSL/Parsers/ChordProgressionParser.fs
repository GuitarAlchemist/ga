...
module ChordProgressionParser =
    ...
    let octave: Parser<int, unit> = pint32 .>> ws
    ...
    let keySpec: Parser<Note, unit> = str "in" >>. ws >>. note
    ...
    let timeSignature: Parser<TimeSignature, unit> =
        pipe3 pint32 (ch '/') pint32 (fun num _ denom -> { Numerator = num; Denominator = denom })

    /// Parse optional time signature
    let optTimeSignature = opt (str "time" >>. ws >>. timeSignature)

    /// Parse tempo
    let tempo: Parser<int, unit> = str "tempo" >>. ws >>. pint32 .>> ws
    ...
    let parseCommand input =
        parse input |> Result.map ChordProgressionCommand
