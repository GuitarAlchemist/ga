...
module MidiParser =
    ...
    let readVLQ (reader: BinaryReader) : int =
        let rec loop acc =
            ...
            if (b &&& 0x80) = 0 then value else loop value
            ...
            if chunkType <> "MTrk" then
                ...
                let rec parseEvents acc =
                    if reader.BaseStream.Position >= endPosition then
                        ...
                        | Result.Ok None -> parseEvents acc
                ...
                | Result.Ok events ->
                    ...
                    let trackName =
                        ...
                        |> List.tryPick (function
                            | TrackName name -> Some name
                            | _ -> None)

                    let instrument =
                        events
                        |> List.tryPick (function
                            | Instrument name -> Some name
                            | _ -> None)
        ...
        | Result.Ok header ->
            ...
            let rec parseTracks trackNum acc =
                if trackNum >= header.TrackCount then
                    ...
                    | Result.Ok track -> parseTracks (trackNum + 1) (track :: acc)
                    | Result.Error err -> Result.Error err

            match parseTracks 0 [] with
            | Result.Ok tracks -> Result.Ok { Header = header; Tracks = tracks }
            | Result.Error err -> Result.Error err
