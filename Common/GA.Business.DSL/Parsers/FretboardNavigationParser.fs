...
module FretboardNavigationParser =
    ...
    let stringNumber: Parser<int, unit> =
        ...
        >>= fun n ->
            if n >= 1 && n <= 12 then
                ...
                fail $"Invalid string number: %d{n} (must be 1-12)"
    ...
    let fretNumber: Parser<int, unit> =
        ...
        >>= fun n ->
            if n >= 0 && n <= 24 then
                ...
                fail $"Invalid fret number: %d{n} (must be 0-24)"
    ...
    let fretboardPosition: Parser<FretboardPosition, unit> =
        choice
            [
              ...
              attempt (
                  pipe3 (str "string" >>. stringNumber) (str "fret" >>. fretNumber) (opt finger) (fun s f fing ->
                      { String = s; Fret = f; Finger = fing })
              )

              // Format: "3:5" (string:fret)
              attempt (pipe3 stringNumber (ch ':') fretNumber (fun s _ f -> { String = s; Fret = f; Finger = None }))

              // Format: "position 5 on string 3"
              attempt (
                  pipe2 (str "position" >>. fretNumber) (str "on" >>. str "string" >>. stringNumber) (fun f s ->
                      { String = s; Fret = f; Finger = None })
              )

              // Format: "position 5 3" (position fret string)
              pipe2 (str "position" >>. fretNumber) stringNumber (fun f s -> { String = s; Fret = f; Finger = None }) ]
    ...
    let cagedShape: Parser<CAGEDShape * int, unit> =
        pipe2
            (str "CAGED" >>. str "shape" >>. cagedLetter)
    ...
    let moveCommand: Parser<NavigationCommand, unit> =
        pipe2 (str "move" >>. direction) (opt pint32) (fun dir dist -> Move(dir, defaultArg dist 1))

    /// Parse a slide command
    let slideCommand: Parser<NavigationCommand, unit> =
        pipe3 (str "slide" >>. str "from" >>. fretboardPosition) (str "to") fretboardPosition (fun from _ ``to`` ->
            Slide(from, ``to``))

    /// Parse a navigation command
    let navigationCommand: Parser<NavigationCommand, unit> =
        choice [ attempt slideCommand; attempt gotoShape; attempt moveCommand; gotoPosition ]
    ...
    let parseCommand input =
        parse input |> Result.map NavigationCommand
