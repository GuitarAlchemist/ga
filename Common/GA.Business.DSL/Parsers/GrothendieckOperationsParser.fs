...
module GrothendieckOperationsParser =
    ...
    let romanNumeral: Parser<string, unit> =
        ...
        let suffix =
            opt (choice [ str "maj7"; str "min7"; pstring "°"; pstring "ø"; pstring "+"; pstring "7" ])
    ...
    let musicalObject, musicalObjectRef =
        createParserForwardedToRef<MusicalObject, unit> ()

    /// Parse chord list
    let chordList: Parser<MusicalObject list, unit> = sepBy1 chordObject (ch ',')

    /// Parse roman numeral list
    let romanNumeralList: Parser<string list, unit> = sepBy1 romanNumeral (ch '-')

    /// Parse progression object
    let progressionObject: Parser<MusicalObject, unit> =
        choice
            [ between (ch '{') (ch '}') chordList
              |>> (fun chords ->
                  let progChords =
                      chords
                      |> List.choose (function
                          | ChordObject c -> Some(AbsoluteChord c)
                          | _ -> None)
              ...
              |>> (fun numerals ->
                  ProgressionObject
                      { Chords = []
                        ...
                        TimeSignature = None
                        Tempo = None
                        Metadata = Map.empty }) ] // TODO: Convert roman numerals to chords
    ...
    /// Parse position list
    let positionList: Parser<(int * int) list, unit> = sepBy1 position (ch ',')

    /// Parse voicing object
    let voicingObject: Parser<MusicalObject, unit> =
        between (ch '{') (ch '}') positionList
        |>> (fun positions ->
            VoicingObject(
                positions
                |> List.map (fun (str, fret) ->
                    { String = str
                      Fret = fret
                      Finger = None })
            ))
    ...
    let pitchClass: Parser<int, unit> =
        ...
        >>= (fun n ->
            if n >= 0 && n <= 11 then
                ...
                fail "Pitch class must be between 0 and 11")

    /// Parse pitch class list
    let pitchClassList: Parser<int list, unit> = sepBy1 pitchClass (ch ',')
                  ...
                  (fun _ name mappings (src, tgt) ->
                      DefineFunctor
                          { Name = name
                            SourceCategory = src
                            TargetCategory = tgt
                            MorphismMappings = Some mappings }) ]
    ...
    /// Parse functor list
    let functorList: Parser<FunctorName list, unit> = sepBy1 identifier (ch ',')
    ...
    let natTransComponent: Parser<NatTransComponent, unit> =
        pipe3 musicalObject (str "->") morphismExpression (fun obj _ morph -> { Object = obj; Morphism = morph })
    ...
    /// Parse object list
    let objectList: Parser<MusicalObject list, unit> = sepBy1 musicalObject (ch ',')
    ...
    let colimitOperation: Parser<GrothendieckOperation, unit> =
        choice
            [
              ...
              pipe3 musicalObject (str "pushout") morphismExpression (fun obj _ morph -> Pushout(obj, morph, obj))
    ...
    let toposOperation: Parser<GrothendieckOperation, unit> =
        choice
            [
              ...
              str "truth_value" >>. ws1 >>. str "of" >>. ws1 >>. musicalObject
              |>> (fun obj -> SubobjectClassifier(Some obj))
              // P(object) or power(object)
              (str "P" <|> str "power") >>. between (ch '(') (ch ')') musicalObject
              |>> PowerObject
              // subobjects of object
              str "subobjects" >>. ws1 >>. str "of" >>. ws1 >>. musicalObject |>> PowerObject
              // Hom(obj1, obj2)
              pipe5 (str "Hom" >>. ch '(') musicalObject (ch ',') musicalObject (ch ')') (fun _ obj1 _ obj2 _ ->
    ...
    let sheafSection: Parser<SheafSection, unit> =
        pipe3 identifier (str "->") musicalObject (fun openSet _ obj -> { OpenSet = openSet; Value = obj })

    /// Parse section mapping
    let sectionMapping: Parser<SheafSection list, unit> = sepBy1 sheafSection (ch ';')

    /// Parse gluing rule
    let gluingRule: Parser<GluingRule, unit> =
        pipe5 identifier (pstring "∩") identifier (str "->") morphismExpression (fun set1 _ set2 _ morph ->
            { OpenSet1 = set1
              OpenSet2 = set2
              Morphism = morph })
    ...
    /// Parse section list
    let sectionList: Parser<MusicalObject list, unit> = sepBy1 musicalObject (ch ',')
    ...
    let sheafOperation: Parser<GrothendieckOperation, unit> =
        choice
            [
              // sheaf name on space
              pipe4 (str "sheaf") identifier (str "on") spaceSpec (fun _ name _ space ->
                  DefineSheaf
                      { Name = name
                        Space = space
                        Sections = None })
              ...
              pipe3
                  identifier
                  (pstring "|" <|> (str "restrict" >>. ws1 >>. str "to" >>. ws1 >>% "|"))
                  identifier
                  (fun sheaf _ openSet -> SheafRestriction(sheaf, openSet))
              // glue { sections } or glue sections along gluing_data
              choice
                  [ str "glue" >>. ws1 >>. between (ch '{') (ch '}') sectionList
                    |>> (fun sections -> SheafGluing(sections, None))
                    pipe4 (str "glue") sectionList (str "along") gluingData (fun _ sections _ rules ->
                        SheafGluing(sections, Some rules)) ] ]
    ...
    let tensorProduct: Parser<GrothendieckOperation, unit> =
        choice
            [
              ...
              // tensor(obj1, obj2)
              pipe5 (str "tensor" >>. ch '(') musicalObject (ch ',') musicalObject (ch ')') (fun _ obj1 _ obj2 _ ->
    ...
    let directSum: Parser<GrothendieckOperation, unit> =
        choice
            [
              ...
              pipe5 (str "direct_sum" >>. ch '(') musicalObject (ch ',') musicalObject (ch ')') (fun _ obj1 _ obj2 _ ->
                  DirectSum(obj1, obj2)) ]
    ...
    let exponentialOp: Parser<GrothendieckOperation, unit> =
        choice
            [
              ...
              // exp(obj1, obj2)
              pipe5 (str "exp" >>. ch '(') musicalObject (ch ',') musicalObject (ch ')') (fun _ obj1 _ obj2 _ ->
    ...
    let parseCommand input =
        parse input |> Result.map GrothendieckCommand
