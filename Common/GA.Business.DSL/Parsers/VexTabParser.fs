...
module VexTabParser =
    ...
    let pboolNoNewline: Parser<bool, unit> =
        choice [ pstring "true" >>% true; pstring "false" >>% false ]

    /// Parse an integer without consuming newlines
    let pintNoNewline: Parser<int, unit> = pint32 .>> hws
    ...
    /// Parse an octave
    let octave: Parser<int, unit> = digit |>> (fun c -> int c - int '0') .>> ws
    ...
    let stringNumber: Parser<int, unit> = digit |>> (fun c -> int c - int '0') .>> ws

    /// Parse a fret without consuming newlines
    let fretNoNewline: Parser<Fret, unit> =
        choice [ pchar 'X' >>% Muted; pintNoNewline |>> FretNumber ]

    /// Parse a string number without consuming newlines
    let stringNumberNoNewline: Parser<int, unit> = digit |>> (fun c -> int c - int '0')
    ...
    let duration: Parser<Duration, unit> =
        ch ':'
        >>. pipe3 durationCode (opt (ch 'd')) (opt (ch 'S')) (fun code dotted slash ->
            { Code = code
    ...
    /// Parse a hammer-on
    let hammerOn: Parser<Technique, unit> = ch 'h' >>. pint |>> HammerOn

    /// Parse a pull-off
    let pullOff: Parser<Technique, unit> = ch 'p' >>. pint |>> PullOff

    /// Parse a slide
    let slide: Parser<Technique, unit> = ch 's' >>. pint |>> Slide

    /// Parse a bend
    let bend: Parser<Technique, unit> =
        ch 'b'
        >>. pipe2 pint (opt (ch 'b' >>. pint)) (fun toFret release -> Bend(toFret, release))
    ...
    /// Parse tap
    let tap: Parser<Technique, unit> = ch 't' >>% Tap

    /// Parse stroke
    let stroke: Parser<Technique, unit> =
        choice [ ch 'u' >>% Upstroke; ch 'd' >>% Downstroke ]
    ...
    let techniqueChain: Parser<Technique list, unit> = many technique
    ...
    let articulation: Parser<Articulation, unit> =
        between
            (str "$")
            (str ".$")
            (pipe2 articulationType (ch '/' >>. articulationPosition) (fun artType pos ->
                { Type = artType; Position = pos }))

    /// Parse optional articulation
    let optArticulation: Parser<Articulation option, unit> = opt articulation
    ...
    let tabNote: Parser<TabNote, unit> =
        pipe5 fret (ch '/' >>. stringNumber) techniqueChain optArticulation (preturn ()) (fun fretVal str techs art _ ->
            { Fret = fretVal
              String = str
              Techniques = techs
              Articulation = art })
    ...
    let chordNote: Parser<ChordNote, unit> =
        choice [ attempt (standardNote |>> StandardChordNote); tabNote |>> TabChordNote ]

    /// Parse a chord
    let chord: Parser<Chord, unit> =
        between (ch '(') (ch ')') (sepBy1 chordNote (ch '.')) .>>. techniqueChain
        |>> (fun (notes, techs) -> { Notes = notes; Techniques = techs })

    /// Parse a rest
    let rest: Parser<Rest, unit> =
        between (ch '#') (ch '#') (opt (digit |>> (fun c -> int c - int '0')))
    ...
    let keySignature: Parser<KeySignature, unit> =
        pipe3 noteLetter optAccidental (opt (choice [ str "major"; str "minor" ])) (fun root acc mode ->
            { Root = root
              Accidental = acc
              Mode = mode })
    ...
    let tuningNote: Parser<NoteLetter * VexAccidental option * int, unit> =
        pipe3 noteLetter optAccidental (ch '/' >>. octave) (fun letter acc oct -> (letter, acc, oct))
    ...
    let tabstaveOptionParser: Parser<TabstaveOptions -> TabstaveOptions, unit> =
        skipMany1 (pchar ' ' <|> pchar '\t')
        >>. choice
                [ pstring "notation=" >>. pboolNoNewline
                  |>> (fun v opts -> { opts with Notation = Some v })
                  pstring "tablature=" >>. pboolNoNewline
    ...
    let tabstaveLine: Parser<TabstaveOptions, unit> =
        ...
        |>> (fun updates -> List.fold (fun opts update -> update opts) defaultTabstave updates)
    ...
    /// Parse options line
    let optionsLine: Parser<VexOption list, unit> = str "options" >>. many vexOption
    ...
    let positionModifier: Parser<int, unit> = ch '.' >>. pint

    /// Parse font modifier
    let fontModifier: Parser<string * int * string, unit> =
        str ".font="
        >>. pipe3 identifier (ch '-' >>. pint) (ch '-' >>. identifier) (fun face size style -> (face, size, style))
    ...
    let textItem: Parser<TextItem, unit> =
        choice
            [ str "++" >>% NewLine
              ...
              manySatisfy (fun c -> c <> ',' && c <> '|' && c <> '#' && c <> '+')
              |>> TextString ]

    /// Parse text line
    let textLine: Parser<Duration option * TextItem list, unit> =
        str "text"
        >>. pipe2 optDuration (sepBy textItem (ch ',')) (fun dur items -> (dur, items))
    ...
    let notesLine: Parser<Duration option * NoteItem list, unit> =
        pstring "notes"
        >>. hws
        >>. pipe2 optDuration (sepBy noteItemNoNewline (choice [ pchar '-' >>% (); hws1 ])) (fun dur items ->
            (dur, items))
    ...
    let textStyle: Parser<TextStyle, unit> =
        between
            ...
            (choice
                [ attempt (
                      pipe3 identifier (ch '-' >>. pint) (ch '-' >>. identifier) (fun face size style ->
                          Custom(face, size, style))
                  )
                  identifier |>> Preset ])
    ...
    let annotationParser: Parser<Annotation, unit> =
        between
            (ch '$')
            (ch '$')
            (pipe2 (opt textStyle) (manySatisfy (fun c -> c <> '$')) (fun style text -> { Style = style; Text = text }))
