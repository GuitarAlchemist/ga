...
module AsciiTabParser =
    ...
    /// Parse fret number
    let fretNumber: Parser<Fret, unit> = pint |>> FretNumber

    /// Parse muted string
    let muted: Parser<Fret, unit> = (pchar 'x' <|> pchar 'X') >>% Muted

    /// Parse open string
    let openString: Parser<Fret, unit> = pchar '0' >>% Open

    /// Parse fret
    let fret: Parser<Fret, unit> =
        choice [ attempt muted; attempt openString; fretNumber ]
    ...
    let hammerOn: Parser<Technique, unit> =
        pipe3 fret (pchar 'h' <|> pchar 'H') fret (fun from _ to' -> HammerOn(from, to'))

    /// Parse pull-off
    let pullOff: Parser<Technique, unit> =
        pipe3 fret (pchar 'p' <|> pchar 'P') fret (fun from _ to' -> PullOff(from, to'))

    /// Parse slide up
    let slideUp: Parser<Technique, unit> =
        pipe3 fret (pchar '/' <|> pchar 's' <|> pchar 'S') fret (fun from _ to' -> SlideUp(from, to'))

    /// Parse slide down
    let slideDown: Parser<Technique, unit> =
        pipe3 fret (pchar '\\' <|> pchar 's' <|> pchar 'S') fret (fun from _ to' -> SlideDown(from, to'))

    /// Parse bend
    let bend: Parser<Technique, unit> =
        pipe2 fret ((pchar 'b' <|> pchar '^') >>. opt fret) (fun from to' -> Bend(from, to'))

    /// Parse bend and release
    let bendRelease: Parser<Technique, unit> =
        pipe4 fret (pchar 'b') fret (pchar 'r' >>. fret) (fun from _ bendTo releaseTo ->
            BendRelease(from, bendTo, releaseTo))

    /// Parse vibrato
    let vibrato: Parser<Technique, unit> =
        pipe2 fret (pchar '~' <|> pchar 'v' <|> pchar 'V') (fun f _ -> Vibrato f)

    /// Parse harmonic
    let harmonic: Parser<Technique, unit> =
        between (pchar '<') (pchar '>') fret <|> between (pchar '(') (pchar ')') fret
        |>> Harmonic

    /// Parse artificial harmonic
    let artificialHarmonic: Parser<Technique, unit> =
        between (pchar '[') (pchar ']') fret |>> ArtificialHarmonic

    /// Parse pinch harmonic
    let pinchHarmonic: Parser<Technique, unit> =
        (pstring "PH" <|> pstring "ph") >>. fret |>> PinchHarmonic

    /// Parse tap
    let tap: Parser<Technique, unit> = (pchar 't' <|> pchar 'T') >>. fret |>> Tap

    /// Parse trill
    let trill: Parser<Technique, unit> =
        pipe3 fret (pstring "tr" <|> pstring "TR") fret (fun f1 _ f2 -> Trill(f1, f2))

    /// Parse pre-bend
    let preBend: Parser<Technique, unit> =
        between (pchar '(') (pchar ')') (fret .>> pchar 'b') |>> PreBend

    /// Parse ghost note
    let ghostNote: Parser<Technique, unit> =
        between (pchar '(') (pchar ')') fret |>> GhostNote

    /// Parse dead note
    let deadNote: Parser<Technique, unit> = (pchar 'x' <|> pchar 'X') >>% DeadNote
    ...
    let stringLine: Parser<StringLine, unit> =
        pipe3 stringName (pchar '|') (many stringContentItem) (fun name _ content ->
            { StringName = name; Content = content })
    ...
    /// Parse tempo
    let tempo: Parser<Annotation, unit> = pstring "Tempo:" >>. ws >>. pint |>> Tempo

    /// Parse time signature
    let timeSignature: Parser<Annotation, unit> =
        pipe3 pint (pchar '/') pint (fun num _ denom -> TimeSignature { Numerator = num; Denominator = denom })

    /// Parse capo
    let capo: Parser<Annotation, unit> = pstring "Capo:" >>. ws >>. pint |>> Capo
    ...
    /// Parse repeat
    let repeat: Parser<Annotation, unit> = pchar 'x' >>. pint |>> Repeat
