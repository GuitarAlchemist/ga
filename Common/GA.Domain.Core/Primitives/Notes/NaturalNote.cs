    ...
    IPitchClass
{
    ...
    public static NaturalNote operator ++(NaturalNote naturalNote) => FromValue(WrapValue(naturalNote.Value + 1));

    public static NaturalNote operator --(NaturalNote naturalNote) => FromValue(WrapValue(naturalNote.Value - 1));

    public static NaturalNote operator +(NaturalNote naturalNote, SimpleIntervalSize intervalSize) =>
        FromValue(WrapValue(naturalNote.Value + intervalSize.Value - 1));

    public static SimpleIntervalSize operator -(NaturalNote endNote, NaturalNote startNote) =>
        NaturalNoteIntervals.Get(new(startNote, endNote));

    public static int CheckRange(int value) =>
        ValueObjectUtils<NaturalNote>.EnsureValueRange(value, _minValue, _maxValue);

    public static int CheckRange(int value, int minValue, int maxValue) =>
        ValueObjectUtils<NaturalNote>.EnsureValueRange(value, minValue, maxValue);
