...
public static class PitchClassExtensions
{
    ...
    /// </summary>
    /// <returns>The <see cref="IntervalStructure" />.</returns>
    public static IntervalStructure ToIntervalStructure<T>(this IEnumerable<T> pitchClassObjects) where T : IPitchClass => pitchClassObjects.Select(item => item.PitchClass).ToIntervalStructure();
}
