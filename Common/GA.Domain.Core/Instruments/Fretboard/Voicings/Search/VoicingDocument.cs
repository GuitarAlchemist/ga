namespace GA.Domain.Core.Instruments.Fretboard.Voicings.Search;
...
public record VoicingDocument
{
    ...
    public string? Quality { get; init; } // Phase 3
    ...
    public static VoicingDocument FromAnalysis(
        ...
        string tuningId = AnalysisConstants.DefaultTuning,
        int capo = 0,
        ...
        int translationOffset = 0)
    {
        ...
        return new()
        {
            ...
            // Identifiers
            TuningId = tuningId,
            PitchClassSetId = primeFormId ?? analysis.EquivalenceInfo?.PrimeFormId ?? AnalysisConstants.Unknown,
            ...
            RootPitchClass = analysis.MidiNotes.Length > 0 ? analysis.MidiNotes[0] % 12 : 0, // Approx
            ...
            Inversion = CalculateInversion(
                analysis.MidiNotes.Length > 0 ? analysis.MidiNotes[0] : 0, 
                analysis.ChordId.RootPitchClass != null ? int.Parse(analysis.ChordId.RootPitchClass) : 0),
            ...
            ForteCode = analysis.EquivalenceInfo?.ForteCode ?? (ForteCatalog.TryGetForteNumber(analysis.PitchClassSet.PrimeForm, out var forte) ? forte.ToString() : null),
            TranslationOffset = translationOffset != 0 ? translationOffset : analysis.EquivalenceInfo?.TranslationOffset ?? 0,
            
            MidiNotes = [.. analysis.MidiNotes.Select(n => n)],
    ...
    private static string BuildSearchableText(MusicalVoicingAnalysis analysis, string diagram)
    {
        ...
        if (analysis.AlternateChordNames != null && analysis.AlternateChordNames.Length > 0)
        {
            sb.Append(' ');
    ...
    private static string BuildYamlAnalysis(Voicing voicing, MusicalVoicingAnalysis analysis)
    {
        ...
        var midiNotes = string.Join(", ", analysis.MidiNotes.Select(n => n));
        ...
        sb.AppendLine("chord:");
        sb.AppendLine($"  name: \"{analysis.ChordId.ChordName ?? AnalysisConstants.Unknown}\"");
        sb.AppendLine($"  key_function: \"{analysis.ChordId.FunctionalDescription ?? AnalysisConstants.FunctionalDescriptionUnknown}\"");
        ...
        if (analysis.AlternateChordNames?.Length > 0)
        {
