...
public readonly record struct Fret : IStaticValueObjectList<Fret>
{
    ...
    ///     <see cref="Muted" />.
    /// </remarks>
    public Fret([ValueRange(_minValue, _maxValue)] int value) => _value = CheckRange(value);
    ...
    public static Result<Fret, string> TryCreate(int value) => value is < _minValue or > _maxValue
        ...
        : Result<Fret, string>.Success(new() { Value = value });
    ...
    public static ImmutableSortedSet<Fret> Set(params ReadOnlySpan<int> values) =>
        values.Length == 0 ? [] : [.. values.ToArray().Select(FromValue)];
