    ...
    IEquatable<IntervalClassVector>
{
    ...
    ///     See
    ///     https://ftp.isdi.co.cu/Biblioteca/BIBLIOTECA%20UNIVERSITARIA%20DEL%20ISDI/COLECCION%20DE%20LIBROS%20ELECTRONICOS/LE-1433/LE-1433.pdf
    ///     - Page 42 (Modulation, Common Tones, and the Deep Scale)
    ...
    public int this[IntervalClass intervalClass] =>
        Vector.TryGetValue(intervalClass, out var value) ? value : 0;
    ...
    public static bool TryParse(string? s, IFormatProvider? provider, out IntervalClassVector result)
    {
        ...
        {
            return false; // Must have exactly 6 interval class counts
        }

        var countByIntervalClass = new Dictionary<IntervalClass, int>();
        for (var i = 0; i < segments.Length; i++)
        {
            if (!int.TryParse(segments[i], out var count))
            {
                return false;
            }
