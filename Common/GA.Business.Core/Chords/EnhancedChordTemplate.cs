...
{
    private readonly ChordTemplate _coreTemplate;
    private readonly HashSet<Scale> _associatedScales = new();
    private readonly Dictionary<PitchClass, ChordTone[]> _chordTonesCache = new();

    /// <summary>Gets the core chord template</summary>
    public ChordTemplate CoreTemplate => _coreTemplate;

    /// <summary>Gets the pitch class set</summary>
    public PitchClassSet PitchClassSet => _coreTemplate.PitchClassSet;

    /// <summary>Gets the chord name</summary>
    public string Name => _coreTemplate.Name;

    /// <summary>Gets the chord formula</summary>
    public ChordFormula Formula => _coreTemplate.Formula;

    /// <summary>Gets the chord quality</summary>
    public ChordQuality Quality => _coreTemplate.Quality;

    /// <summary>Gets the chord extension</summary>
    public ChordExtension Extension => _coreTemplate.Extension;

    /// <summary>Gets the stacking type</summary>
    public ChordStackingType StackingType => _coreTemplate.StackingType;
    ...
    {
        _coreTemplate = coreTemplate ?? throw new ArgumentNullException(nameof(coreTemplate));
    }
    ...
    {
        _coreTemplate = new ChordTemplate(pitchClassSet, name);
    }
    ...
    public bool IsCompatibleWith(Scale scale)
    {
        ...
        return _coreTemplate.IsCompatibleWith(scale.PitchClassSet);
    }
    ...
    public IEnumerable<Scale> GetContainingScales()
    {
        return _associatedScales.Where(scale => 
            _coreTemplate.PitchClassSet.IsSubsetOf(scale.PitchClassSet));
    }
    ...
    public ChordVoicing CreateVoicing(PitchClass root, PitchClass? bass = null)
    {
        ...
        return new ChordVoicing(_coreTemplate, chordTones, bass ?? root);
    }
    ...
    {
        if (!IsCompatibleWith(scale))
            return Enumerable.Empty<PitchClass>();
    ...
    private ChordTone[] GenerateChordTones(PitchClass root)
    {
        ...
        foreach (var interval in _coreTemplate.Intervals)
        {
    ...
    {
        var chordPitchClasses = _coreTemplate.PitchClassSet.Count;
        var scalePitchClasses = scale.PitchClassSet.Count;
        var commonPitchClasses = _coreTemplate.PitchClassSet.Intersect(scale.PitchClassSet).Count();
    ...
    public bool Equals(EnhancedChordTemplate? other)
    {
        ...
        return _coreTemplate.Equals(other._coreTemplate);
    }

    public override bool Equals(object? obj) => Equals(obj as EnhancedChordTemplate);
    public override int GetHashCode() => _coreTemplate.GetHashCode();
...
public class ChordVoicing
{
    ...
    public override string ToString() => 
        $"{ChordTemplate.Name}{(IsInverted ? $"/{Bass}" : "")}";
}
