...
public sealed class ScaleModeSimpleInterval(SimpleIntervalSize size, IntervalQuality quality, IntervalQuality refQuality) 
    : ScaleModeIntervalBase<SimpleIntervalSize>(size, quality, refQuality)
{
    ...
    public ScaleModeCompoundInterval ToCompound() => new(Size.ToCompound(), Quality, RefQuality);
}