...
public abstract partial record Interval : IComparable<Interval>, IComparable
{
    ...
    public int CompareTo(object? obj)
    {
        ...
        return obj is Interval other ? CompareTo(other) : throw new ArgumentException($"Object must be of type {nameof(Interval)}");
    }

    public static bool operator <(Interval? left, Interval? right) => Comparer<Interval>.Default.Compare(left, right) < 0;
    public static bool operator >(Interval? left, Interval? right) => Comparer<Interval>.Default.Compare(left, right) > 0;
    public static bool operator <=(Interval? left, Interval? right) => Comparer<Interval>.Default.Compare(left, right) <= 0;
    public static bool operator >=(Interval? left, Interval? right) => Comparer<Interval>.Default.Compare(left, right) >= 0;
    ...
    public sealed partial record Simple : Diatonic, IParsable<Simple>, IFormattable
    {
        ...
        public static bool TryParse(string? s, IFormatProvider? provider, out Simple result)
        {
            ...
            if (!SimpleIntervalSize.TryParse(number, null, out var size)) return false; // Failure
            ...
            if (string.IsNullOrEmpty(prefix) || !Accidental.TryParse(prefix, null, out var accidental)) accidental = Accidental.Natural;
...
#pragma warning restore IDE1006 // Naming Styles
        ...
        public override FormulaSimpleInterval ToFormulaInterval() => new(Size, Quality);
    ...
    public sealed partial record Compound : Diatonic, IParsable<Compound>, IFormattable
    {
        ...
        public static bool TryParse(string? s, IFormatProvider? provider, out Compound result)
        {
            ...
            if (!CompoundIntervalSize.TryParse(number, null, out var size)) return false; // Failure
            if (IntervalQuality.TryParse(prefix, null, out var quality))
            {
                // Success
                result = new() { Size = size, Quality = quality };
...
#pragma warning restore IDE1006 // Naming Styles
        ...
        public override FormulaCompoundInterval ToFormulaInterval() => new(Size, Quality);
