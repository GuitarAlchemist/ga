    ...
    IPitchClass
{
    ...
    public sealed record Chromatic(int Value) : Note
    {
        ...
        {
            return ToSharp().ToAccidented();
        }

        public Sharp ToSharp()
        {
        ...
        private class ByValueIndexer() : LazyIndexerBase<int, Chromatic>(GetDictionary())
        {
            ...
            public static Chromatic GetChromaticNote(int value)
            {
                return _instance[value];
            }

            private static ImmutableDictionary<int, Chromatic> GetDictionary()
            {
                return Items.ToImmutableDictionary(note => note.Value);
            }
        }
        ...
        public static Chromatic Get(int value)
        {
            return ByValueIndexer.GetChromaticNote(value);
        }
        ...
        public static implicit operator Chromatic(int value)
        {
            return Get(value);
        }
    ...
    /// <summary>
    ///     A note from a musical key (<see cref="Note.KeyNote.Sharp" /> | <see cref="Note.KeyNote.Flat" />  |
    ///     <see cref="Note.KeyNote.Accidented" />)
    /// </summary>
    ...
    public abstract record KeyNote(NaturalNote NaturalNote) : Note, IComparable<KeyNote>
    {
        ...
        public static bool TryParse(string? input, out IReadOnlyCollection<KeyNote> keyNotes)
        {
            ...
            if (Sharp.TryParse(input, null, out var sharpKeyNote))
            {
                builder.Add(sharpKeyNote);
            }

            if (Flat.TryParse(input, null, out var flatKeyNote))
            {
                builder.Add(flatKeyNote);
            }
        ...
        public static bool operator <(KeyNote? left, KeyNote? right)
        {
            return Comparer<KeyNote>.Default.Compare(left, right) < 0;
        }
    ...
    public sealed record Sharp(NaturalNote NaturalNote, SharpAccidental? SharpAccidental = null)
        : KeyNote(NaturalNote), IStaticReadonlyCollection<Sharp>, IParsable<Sharp>
    {
        ...
        public static implicit operator Chromatic(Sharp sharp)
        {
            return sharp.ToChromatic();
        }

        public static ImmutableArray<Sharp> Create(params Sharp[] notes)
        {
            return [.. notes];
        }
        ...
        public override string ToString()
        {
            return SharpAccidental.HasValue
        ...
        protected override PitchClass GetPitchClass()
        {
            ...
            if (SharpAccidental.HasValue)
            {
                value += SharpAccidental.Value.Value;
            }
    ...
    public sealed record Flat(NaturalNote NaturalNote, FlatAccidental? FlatAccidental = null)
        : KeyNote(NaturalNote), IStaticReadonlyCollection<Flat>, IParsable<Flat>
    {
        /// <inheritdoc />
        public override PitchClass PitchClass => GetPitchClass();
        ...
        public override string ToString()
        {
            return FlatAccidental.HasValue
        ...
        {
            var value = NaturalNote.PitchClass.Value;
            if (FlatAccidental.HasValue)
            {
                value += FlatAccidental.Value.Value;
            }
        ...
        : Note, IStaticReadonlyCollection<Accidented>, IParsable<Accidented>
    {
        ...
        {
            return note1.GetInterval(note2);
        }
        ...
        public static bool TryParse(string? s, IFormatProvider? provider, out Accidented result)
        {
            ...
            for (var i = Math.Min(2, lengthOfNote); i > 0; i--)
            {
                ...
                var notePart = s[..i];
                if (!NaturalNote.TryParse(notePart, null, out var parsedNote))
                {
                    continue;
                }
                ...
                if (!string.IsNullOrEmpty(accidentalPart))
                {
                    ...
                    if (!Intervals.Primitives.Accidental.TryParse(accidentalPart, null, out var parsedAccidental))
                    {
                        return false; // If there's an accidental part but it can't be parsed
                    }
    ...
    {
        return Comparer<Note>.Default.Compare(left, right) < 0;
    }
