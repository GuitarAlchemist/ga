...
Research and define the architecture for an ML model that improves the "Naturalness" of generative tablature realization
beyond raw physical heuristics.
...
* **Innovation**: Uses separate prediction heads for string selection and fret selection, constrained by instrument
  geometry.

### 2. Tablature as Language (Seq2Seq)

* Treating guitar tablature as a "target language" allows the model to learn **idiomatic transitions** (e.g. why a
  player prefers a specific finger slide over a string jump).
* **Bi-directional context**: Models like BERT (Encoder-only) are excellent for "Naturalness Ranking" (scoring a full
  sequence), while GPT-style (Decoder-only) models are better for "Generative Realization".
...
* **Benefit**: Can find solutions that heuristics might miss (e.g. intentional "open-string ringing" or stylistic
  quirks).
...
3. **Training**:
    * Train a small **DistilBERT-style** model on the "Naturalness" task (Binary classification: Human vs. Randomized
      Tab).
4. **Integration**:
...
The **"Naturalness Ranker" (Strategy A)** is the most pragmatic next step. It complements our robust Viterbi solver
without requiring a massive generative model shift.
