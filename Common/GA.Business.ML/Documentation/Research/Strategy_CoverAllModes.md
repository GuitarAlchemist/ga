...
To provide a unified system that can identify and classify *any* possible collection of notes, bridging the gap between
traditional **Tonal Modes** (e.g., Lydian, Dorian) and structural **Atonal Families** (Set Classes/Forte Numbers).
...
- **Prioritization:** "Standard" modes (Church modes) are preferred over "Exotic" modes when both match, unless the
  exotic match is significantly better.
...
- **Completed:** Updated `ModalFlavorService` to use conflict-based scoring (Score = Matches - Conflicts) and priority
  tiers to favor standard names.

### 2. Integrate Atonal Identification

- **Next Step:** Update `AutoTaggingService` (and `TabAnalysisService`) to compute and store the **Forte Number** for
  every analyzed voicing.
- **Result:** Every voicing will have a structural tag (e.g., `Set:7-35`) regardless of whether it has a semantic flavor
  name.
