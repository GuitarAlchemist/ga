...
* **Gap**: "Flavor" tags (Lydian, Dorian) are currently handled as *string metadata* in `VoicingDocument.SemanticTags`.
  They are **not** encoded in the `BitVector` (dimensions 0-63).
* **Consequence**: We cannot currently perform *vector-based* retrieval for specific modal flavors; retrieval relies on
  keyword filtering or hybrid search.
