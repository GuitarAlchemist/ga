...
public class ModalFlavorService
{
    ...
    private readonly List<ModeFlavorDefinition> _flavors = [];
    ...
    private int GetModePriority(string name)
    {
        ...
        if (name.Contains("Pentatonic", StringComparison.OrdinalIgnoreCase) ||
            name.Contains("Bebop", StringComparison.OrdinalIgnoreCase))
        {
            return 5;
        }
    ...
    public void Enrich(ChordVoicingRagDocument doc, HashSet<string> tags)
    {
        if (doc.RootPitchClass == null || doc.PitchClasses == null || doc.PitchClasses.Length == 0)
        {
            return;
        }
        ...
        foreach (var flavor in _flavors)
        {
            ...
            var matchCount = flavor.CharacteristicSemitones.Count(s => voicingIntervals.Contains(s));
            ...
            if (matchCount > 0)
            {
                ...
                var fullMatchCount = flavor.FullIntervals.Count(s => voicingIntervals.Contains(s));
        ...
        int Priority)
    {
        ...
        public int MatchCount(HashSet<int> voicingIntervals) =>
            CharacteristicSemitones.Count(s => voicingIntervals.Contains(s));
    }
}
