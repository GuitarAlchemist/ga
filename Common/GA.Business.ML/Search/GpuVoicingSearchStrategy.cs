...
public class GpuVoicingSearchStrategy : IVoicingSearchStrategy, IDisposable
{
    ...
    private Action<Index1D, ArrayView<double>, ArrayView<double>, ArrayView<int>, ArrayView<double>, int, int>?
        _filteredCosineSimilarityKernel;
    ...
    public GpuVoicingSearchStrategy() => InitializeIlgpu();
    ...
    private IEnumerable<(string VoicingId, double Score)> CalculateSimilaritiesIlgpu(double[] queryEmbedding)
    {
        ...
        if (_isDisposed || _accelerator.AcceleratorType == AcceleratorType.CPU || _cosineSimilarityKernel == null ||
            _deviceEmbeddings == null)
        {
        ...
        List<string> allowedIds)
    {
        ...
        if (_isDisposed || _accelerator.AcceleratorType == AcceleratorType.CPU ||
            _filteredCosineSimilarityKernel == null || _deviceEmbeddings == null)
        {
            // Fall back to CPU computation if GPU is not available or disposed
