    ...
    public async Task<BenchmarkResult> RunAsync()
    {
        ...
        foreach (var (diagram, expectedName, desc) in groundTruth)
        {
            var step = new BenchmarkStep
                { Name = $"Identify {expectedName}", Input = diagram, Expected = expectedName };
            ...
            else
            {
                ...
                var matches = step.Actual.Equals(expectedName, StringComparison.OrdinalIgnoreCase) ||
                              step.Actual.Replace(" Major", "").Replace(" Minor", "m")
