...
/// </summary>
public class GPUAcceleratedEmbeddingService : ITextEmbeddingService, IBatchTextEmbeddingService, IDisposable
{
        ...
        ILogger<GPUAcceleratedEmbeddingService>? logger = null)
    {
        ...
        _concurrencyLimiter = new(maxConcurrentRequests, maxConcurrentRequests);
        ...
        foreach (var device in _gpuContext.Devices)
        {
                ...
                device.AcceleratorType == AcceleratorType.OpenCL)
            {
                ...
                _logger.LogInformation("{DeviceType} GPU acceleration enabled: {Device}", device.AcceleratorType, _accelerator.Name);
                break;
            }
        }
        ...
        {
            _accelerator = _gpuContext.GetPreferredDevice(preferCPU: true).CreateAccelerator(_gpuContext);
        ...
        for (var i = 0; i < texts.Length; i++)
        {
            ...
            tasks[i] = Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    embedding.Span.CopyTo(targetSpan);
                }
                finally
                {
                    semaphore.Release();
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Copy data to GPU
        _gpuInputBuffer.CopyFromCPU(rawEmbeddings.ToArray());
        ...
        await Task.Run(() =>
        {
            for (var i = 0; i < numEmbeddings; i++)
            {
                ...
                if (magnitude > 0)
                {
                    ...
                    // Normalize on GPU
                    _vectorNormalizeKernel((int)embeddingDimension, embeddingView, outputView, invMagnitude);
                }
            }
    ...
    private float CalculateMagnitudeOnGPU(ArrayView<float> vector)
    {
        ...
        using var tempBuffer = _accelerator.Allocate1D<float>(dimension);

        // Compute squared values on GPU
        _vectorDotProductKernel((int)dimension, vector, vector, tempBuffer.View);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            // Copy data to GPU
            using var queryBuffer = _accelerator.Allocate1D<float>(queryEmbeddings.Length);
            using var docBuffer = _accelerator.Allocate1D<float>(documentEmbeddings.Length);
            using var resultBuffer = _accelerator.Allocate1D<float>(totalComparisons);

            queryBuffer.CopyFromCPU(queryEmbeddings.ToArray());
            docBuffer.CopyFromCPU(documentEmbeddings.ToArray());
