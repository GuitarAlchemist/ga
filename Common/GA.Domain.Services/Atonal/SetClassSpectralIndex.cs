...
{
    private static readonly Lazy<List<(SetClass SetClass, double[] Spectrum)>> _index = new(BuildIndex);

    private static List<(SetClass SetClass, double[] Spectrum)> BuildIndex() =>
    [
        .. SetClass.Items.Select(setClass => (SetClass: setClass, Spectrum: setClass.GetMagnitudeSpectrum()))
    ];
