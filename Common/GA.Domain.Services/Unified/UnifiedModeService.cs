namespace GA.Domain.Services.Unified;
...
{
    private static readonly ConcurrentDictionary<(UnifiedModeId Id, int RotationIndex), UnifiedModeDescription>
        _describeCache = new();

    public UnifiedModeInstance FromScaleMode(ScaleMode mode, PitchClass root)
    {
    ...
    public UnifiedModeInstance FromPitchClassSet(PitchClassSet set, PitchClass root)
    {
        ...
        var family = ModalFamily.TryGetValue(icv, out var fam) && fam.Modes.Count > 1 ? fam : null;
    ...
    /// <inheritdoc />
    public IEnumerable<(UnifiedModeInstance Instance, int Brightness)> RankByBrightness(UnifiedModeClass modeClass,
        PitchClass root)
    {
