...
/// </summary>
public class TabSequenceSolver(FretboardPositionMapper mapper, PhysicalCostService costService)
{
    /// <summary>
    ...
    public List<List<FretboardPosition>> Solve(IEnumerable<IEnumerable<Pitch>> score)
    {
        ...
        var states = steps.Select(pitches => mapper.MapChord(pitches).ToList()).ToList();
    ...
    private List<List<FretboardPosition>> PerformViterbi(List<List<List<FretboardPosition>>> states)
    {
        ...
        {
            var staticCost = costService.CalculateStaticCost(states[0][j]).TotalCost;
            dp[0][j] = (staticCost, -1);
        }
        ...
        for (var i = 1; i < n; i++)
        {
            ...
            {
                var minPrevCost = double.MaxValue;
                var bestPrev = -1;

                var currShape = states[i][curr];
                var currStaticCost = costService.CalculateStaticCost(currShape).TotalCost;

                for (var prev = 0; prev < states[i - 1].Count; prev++)
                {
                    var prevShape = states[i - 1][prev];
                    var transCost = costService.CalculateTransitionCost(prevShape, currShape);

                    var total = dp[i - 1][prev].Cost + transCost + currStaticCost;
        ...
        var minFinalCost = double.MaxValue;
        var lastBest = -1;
