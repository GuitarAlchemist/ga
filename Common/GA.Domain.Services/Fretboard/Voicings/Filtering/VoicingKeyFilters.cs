...
public static class VoicingKeyFilters
{
    ...
    public static async Task<Dictionary<Key, List<Voicing>>> GroupByPrimaryKey(this IAsyncEnumerable<Voicing> voicings)
    {
        ...
        await foreach (var voicing in voicings)
        {
            var analysis = Analysis.VoicingAnalyzer.Analyze(voicing);
            var primaryKey = ParseKey(analysis.ChordId.ClosestKey);
    ...
    {
        if (string.IsNullOrEmpty(keyName)) return Key.Major.C; // Default
        if (Key.Major.TryParse(keyName, out var major)) return major;
        if (Key.Minor.TryParse(keyName, out var minor)) return minor;
        return Key.Major.C;
    }
}
