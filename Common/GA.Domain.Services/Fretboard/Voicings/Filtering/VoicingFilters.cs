...
public static class VoicingFilters
{
    ...
    /// </summary>
    public static bool
        MatchesCriteria(Voicing voicing, MusicalVoicingAnalysis analysis, VoicingFilterCriteria criteria) =>
        MatchesCheapFilters(voicing, criteria) && MatchesExpensiveFilters(voicing, analysis, criteria);
    ...
    private static bool MatchesCheapFilters(Voicing voicing, VoicingFilterCriteria criteria)
    {
        ...
        if (!MatchesFretRange(voicing, criteria.FretRange))
        {
            return false;
        }

        if (!MatchesNoteCount(voicing, criteria.NoteCount))
        {
            return false;
        }
        ...
        VoicingFilterCriteria criteria)
    {
        ...
        if (!MatchesChordType(analysis, criteria.ChordType))
        {
            return false;
        }

        if (!MatchesVoicingType(analysis, criteria.VoicingType))
        {
            return false;
        }

        if (!MatchesCharacteristics(analysis, criteria.Characteristics))
        {
            return false;
        }

        if (!MatchesKeyContext(analysis, criteria.KeyContext))
        {
            return false;
        }
    ...
    {
        if (filter == null || filter == ChordTypeFilter.All)
        {
            return true;
        }
        ...
        {
            ChordTypeFilter.Triads => analysis.MidiNotes.Select(n => n).Distinct().Count() == 3 &&
    ...
    {
        if (filter == null || filter == VoicingTypeFilter.All)
        {
            return true;
        }
    ...
    {
        if (filter == null || filter == VoicingCharacteristicFilter.All)
        {
            return true;
        }
    ...
    private static bool MatchesKeyContext(MusicalVoicingAnalysis analysis, KeyContextFilter? filter) => true;

    private static bool MatchesFretRange(Voicing voicing, FretRangeFilter? filter)
    {
        if (filter == null || filter == FretRangeFilter.All)
        {
            return true;
        }
    ...
    {
        if (filter == null || filter == NoteCountFilter.All)
        {
            return true;
        }

        var noteCount = voicing.Notes.Select(n => n).Distinct().Count();
