...
interface QualityReadmePayload {
  markdown: string;
  source_path: string;
  source_mtime: string;
  fetched_at: string;
...
const QualityCard: React.FC = () => {
  ...
  const [error, setError] = useState<string | null>(null);
  const [readme, setReadme] = useState<QualityReadmePayload | null>(null);
  ...
  useEffect(() => {
    ...
    fetch('/dev-data/quality')
      ...
      .catch((e) => { if (!cancelled) setError(String(e.message ?? e)); });
    fetch('/dev-data/quality-readme')
      .then((r) => {
        if (!r.ok) throw new Error(`HTTP ${r.status}`);
        return r.json() as Promise<QualityReadmePayload>;
      })
      .then((payload) => { if (!cancelled) setReadme(payload); })
      .catch((e) => { if (!cancelled) setReadmeError(String(e.message ?? e)); });
    return () => { cancelled = true; };
  }, []);
  ...
  return (
    <Paper sx={{ p: 2, height: '100%' }}>
      ...
      <Box sx={{ maxHeight: 320, overflow: 'auto', borderBottom: '1px solid', borderColor: 'divider', mb: 1, pb: 1 }}>
      ...
      {/* Quality Trend Report — full markdown render of docs/quality/README.md
          (auto-generated by ix-quality-trend). Inline so the QA tab carries
          the headline + sparkline + per-domain tables, not just a GitHub link. */}
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1, mt: 2 }}>
        <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>Quality Trend Report</Typography>
        {readme && (
          <Typography variant="caption" color="text.secondary">
            source: <code>{readme.source_path}</code> · updated {new Date(readme.source_mtime).toLocaleDateString()}
          </Typography>
        )}
      </Stack>
      ...
      <Box
        sx={{
          ...
          overflow: 'auto',
          fontSize: '0.85rem',
          '& h1': { fontSize: '1.1rem', mt: 1 },
          '& h2': { fontSize: '1rem', mt: 1.5, mb: 0.5, color: 'text.primary' },
          '& h3': { fontSize: '0.92rem', mt: 1 },
          ...
          '& td:nth-of-type(n+2)': { fontFamily: 'monospace', whiteSpace: 'nowrap' },
          '& code': { fontFamily: 'monospace', fontSize: '0.85em', bgcolor: 'action.hover', px: 0.5, borderRadius: 0.5 },
          '& em': { color: 'text.secondary', fontStyle: 'normal', fontSize: '0.85em' },
          '& ul, & ol': { pl: 3, my: 0.5 },
          '& a': { color: 'primary.main' },
        }}
      >
        {readmeError && <Typography color="warning.main" variant="caption">Quality README endpoint: {readmeError}</Typography>}
        {!readme && !readmeError && <CircularProgress size={16} />}
        {readme && <ReactMarkdown remarkPlugins={[remarkGfm]}>{readme.markdown}</ReactMarkdown>}
      </Box>
    </Paper>
  );
};
