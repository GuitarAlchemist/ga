...
const GENERATED_COURSES_KEY = 'streeling-generated-courses';
...
function generatedCourseId(req: CourseRequest): string {
  return `generated-${req.id}`;
}

function buildGeneratedCourse(req: CourseRequest): Course {
  const departmentId = departments.find(d => d.name === req.department)?.id ?? slugify(req.department);
  const moduleId = `${departmentId.slice(0, 3)}-${slugify(req.topic).slice(0, 48)}`;
  const producedBy = req.assignedTo ?? 'Streeling queue';
  const generatedAt = new Date().toISOString();

  return {
    id: generatedCourseId(req),
    title: req.topic,
    department: departmentId,
    fileName: `${moduleId}.generated.md`,
    availableLanguages: ['en'],
    translations: {},
    content: `---
module_id: ${moduleId}
department: ${departmentId}
course: "${req.topic.replace(/"/g, '\\"')}"
level: ${req.level}
estimated_duration: "${req.level === 'phd' ? '45' : req.level === 'advanced' ? '35' : req.level === 'intermediate' ? '25' : '20'} minutes"
...
> **Department of ${req.department}** | Level: ${req.level} | Generated by ${producedBy}

## Objectives

- Define the core vocabulary and mental model for ${req.topic}
- Connect the topic to practical Guitar Alchemist workflows
- Identify common failure modes and how to diagnose them
- Apply the concept through a short practice exercise

---

## 1. Conceptual Map

${req.topic} sits inside ${req.department} as a practical learning module. Start by separating the durable principles from implementation details: what remains true across tools, tunings, models, or workflows is the part worth practicing first.

## 2. Working Method

1. State the musical or engineering problem in one sentence.
2. List the observable signals that would prove progress.
3. Try the smallest useful exercise.
4. Compare the result against the expected sound, behavior, or explanation.

## 3. Practice Exercise

Create one concrete example of ${req.topic}. For guitar topics, play it in at least two positions. For technical topics, write down the input, transformation, and output. For governance topics, name the policy, signal, and decision boundary.

## 4. Assessment

You can mark this module complete when you can explain ${req.topic} without reading notes and can produce one worked example from scratch.

---

*Generated locally by the Streeling University queue on ${generatedAt}.*
`,
  };
}
...
export const CourseViewer: React.FC<CourseViewerProps> = ({ open, onClose }) => {
  ...
  const [generatedCourses, setGeneratedCourses] = useState<Course[]>(() => loadQueue(GENERATED_COURSES_KEY));
  ...
  useEffect(() => { saveQueue(GENERATED_COURSES_KEY, generatedCourses); }, [generatedCourses]);

  useEffect(() => {
    const completed = courseQueue.filter(r => r.status === 'completed');
    if (completed.length === 0) return;

    setGeneratedCourses(prev => {
      const known = new Set(prev.map(c => c.id));
      const additions = completed
        .filter(req => !known.has(generatedCourseId(req)))
        .map(buildGeneratedCourse);
    ...
    progressTicks.forEach((pct, i) => {
      const t = setTimeout(() => {
        const advance = <T extends { status: string; progress?: number }>(r: T, activeStatuses: string[]): T => {
          ...
          return pct >= 100 ? { ...r, progress: pct, status: 'completed' } as T : { ...r, progress: pct };
        };
        if (type === 'course') {
  ...
  const displayedDepartments = useMemo(() => {
    if (generatedCourses.length === 0) return departments;

    const generatedByDepartment = generatedCourses.reduce<Record<string, Course[]>>((acc, course) => {
      (acc[course.department] ??= []).push(course);
      return acc;
    }, {});

    return departments.map(dept => {
      const generated = generatedByDepartment[dept.id] ?? [];
      if (generated.length === 0) return dept;

      const knownIds = new Set(dept.courses.map(course => course.id));
      const newCourses = generated.filter(course => !knownIds.has(course.id));

      return {
        ...dept,
        courseCount: dept.courseCount + newCourses.length,
        courses: [...newCourses, ...dept.courses],
      };
    });
  }, [generatedCourses]);
  ...
  const submitResearchRequest = useCallback(() => {
    ...
    setQueueExpanded(true);
    scheduleSimulation('research', req.id);
    closeDialog();
  }, [drTopic, drDepth, drSources, dialogDept, closeDialog, scheduleSimulation]);
  ...
  const queueTranslation = useCallback((course: Course, lang: string) => {
    ...
    setQueueExpanded(true);
    scheduleSimulation('translation', `tq::${course.id}::${lang}`);
  }, [translationQueue, scheduleSimulation]);
  ...
  const filteredDepartments = useMemo(() => {
    if (!searchQuery.trim()) return displayedDepartments;
  ...
  }, [displayedDepartments, searchQuery]);
          ...
          <div className="course-viewer__header-left">
            ...
            <div>
              ...
              <span className="course-viewer__subtitle">
                ...
                {generatedCourses.length > 0 && <> &middot; {generatedCourses.length} generated</>}
              </span>
            </div>
          </div>
              ...
              <div className="course-viewer__reader">
                <div className="course-viewer__reader-meta">
                  <span className="course-viewer__reader-dept">
                    {displayedDepartments.find(d => d.id === selectedCourse.department)?.name}
                  </span>
                </div>
