...
**v1.8 headline: invariant #25 flipped from 8.4% PASS (67/793) to 100% (793/793).** Under v4-pp + root-boost, same-PC-set voicings across instruments had bit-different STRUCTURE slices because STRUCTURE's `v[rootPitchClass] += 1.0` made the same pitch-class set differ by root. Removing the root-boost and moving root identity to a dedicated 12-dim ROOT similarity partition (weight 0.05) restored **same-pitch-class-set** invariance (octave / voicing / instrument — what #25 tests). NB: this is NOT full transposition (T) invariance — STRUCTURE's pitch-class chroma is T-variant (a chord vs its transposition has STRUCTURE cosine ≈0.5–0.8; measured by `Tools/GaStructureInvariance`); only the ICV + cardinality sub-dims are T/I-invariant.
