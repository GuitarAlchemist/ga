...
def fix_fsharp_core_versions(root_dir="."):
    """Fix FSharp.Core version to use correct format with dot"""
    count = 0
    ...
    for ext in ["*.csproj", "*.fsproj"]:
        for proj_file in Path(root_dir).rglob(ext):
            try:
                ...
                # Replace 10.0.100-rc2.25502.107 with 10.0.100-rc.2.25502.107 (add dot after rc)
                content = re.sub(r'10\.0\.100-rc2\.25502\.107', '10.0.100-rc.2.25502.107', content)
