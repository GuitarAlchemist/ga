...
- **Orchestrateur AFK overnight (pattern `gnhf`, borné cost-doctrine) — WATCH (2026-07-05)** — inspiré de la config agentic de Kun Chen (ex-Meta L8, ByteByteGo, primaire non lue/403 → snippet-derived) : son `gnhf` (« good night, have fun ») est un orchestrateur long-running *dead-simple* pour lancer de gros lots la nuit. Comble un **manque réel** heurté en session : `send_later`/`ScheduleWakeup` indisponibles → pas de self-check-in AFK sur *notre* spine (Jules externe ne couvre que le bulk async). Le reste de sa config (manager-d'agents, plan+valide>code, agent-agnostique, subagents, écrire/refléter dans CLAUDE.md) est **déjà** notre doctrine (Karpathy R1/R4, Ousterhout, delta Pocock harness-agnostic, boucles `/correct`+`/learnings`+`/digest`, où on est même en avance : `/correct` sanitize + CI-valide). **Condition non négociable** : un overnight orchestrator *non borné* est le footgun exact qui a fait exploser le frugal v0.3 (4,74M tokens, ~10×, recharge CA$13,64) — donc plafond de dépense **dur** + subscription-only + annonce de coût dès le jour 1 (gards v0.4/ga#517 en modèle). **Ne PAS voler** sa métrique 30-40 PRs/jour : notre métrique est correct-vérifié-auditable (tribunal + baselines), pas le volume. **Déclencheur** : quand `ScheduleWakeup`/`send_later` deviennent dispo, OU besoin récurrent de lots nocturnes bornés. Two-way door.
...
- **NVIDIA agentic AI (NIM / NeMo / Nemotron / Blueprints) — WATCH, do not adopt (2026-07-05)** — évalué à la demande opérateur. Ne rentre pas dans la doctrine actuelle : NVIDIA impose soit l'auto-hébergement sur GPU (le plan cloud validé est une boîte CPU Hetzner ~8 €/mois), soit l'API pay-per-token via build.nvidia.com (viole *subscription-only, jamais pay-per-use*) ; et remplacer le spine Claude par NIM/NeMo est une réécriture du harness, contre *fiabilité = harness pas vendor*. Le rôle « model-tiering + agents externes » est déjà couvert (Claude Haiku/Sonnet/Opus + Jules async + Codex revue + Ollama local). **Seule exception opt-in, périphérique, off critical path** : modèles open-weight auto-hébergés (Nemotron/NIM) pour du bulk privé à l'échelle, OU un Blueprint spécifique (RAG/video-search) mappant une feature GA. **Déclencheur de réévaluation** : (a) on acquiert du GPU, OU (b) un besoin de bulk privé à l'échelle dépasse ce qu'Ollama+subscription couvrent. Two-way door, coût de report nul.
- **PraisonAI — évalué, NON (2026-07-11, duplication de couche)** — framework d'orchestration multi-agents Python/JS **construit sur le Claude Agent SDK** (README lu) : agents YAML/5-lignes, workflows routing/parallel/loops, memory/RAG, MCP. Verdict : on tourne **déjà sur sa fondation un niveau plus bas** (cette session = Agent SDK natif : Workflow, sub-agents, `.claude/agents/`, skills) — chaque feature a son équivalent natif, souvent plus riche (tribunal, baselines, forecast ledger). Stack mismatch (Python/JS vs Rust+.NET+F#), et les deux surfaces d'orchestration sont déjà tranchées : Claude Code/SDK (harness dev) + MAF (runtime produit). Une 3ᵉ = fragmentation. Timing contre : l'audit Codex (ix#228) dit *consolidation avant nouvelles capacités*. Rien d'unique à voler (agents YAML ≈ agents tars/`.claude/agents/`). Déclencheur de réévaluation : aucun prévu — re-regarder seulement si on abandonne une des deux surfaces choisies.
- **Graduer le spike MAF Agent Skills (.NET) — ACTIONNABLE (2026-07-07)** — Microsoft Agent Framework « Agent Skills for .NET » est passé **stable/released**. Différence nette avec NVIDIA (watch, non) : c'est une **librairie** sur `Microsoft.Extensions.AI` — la fondation que **GaChatbot utilise déjà** (`RoutedChatApplicationService`/`OrchestratedChatApplicationService`/`DefaultChatClientFactory`/`CascadingChatClient`), pas de l'infra payante. Et GA a **déjà** deux prototypes en test-suite : `AgentFrameworkSpikeTests.cs` + `FileBasedSkillsProviderTests.cs`. Le format *file-based* de MAF = **`SKILL.md` + scripts on-demand**, identique aux `.claude/skills/` — l'écosystème en a **232** (ga 41 / ix 103 / Demerzel 69 / tars 11 / hari 8). **Atout compounding = write-once, two-surface** : le même corpus SKILL.md consommé par le harness dev (Claude Code) *et* le runtime produit (.NET/GaChatbot). **Tracer bullet** : graduer le spike off-experimental + charger **UN** skill music-theory dans GaChatbot via le FileBasedSkillsProvider, bout-en-bout, avant d'étendre. **Garde** : simplicity-first — ne pas migrer tout le layer chatbot vers MAF sans besoin démontré. Sources (snippet, primaire 403'd) : devblogs.microsoft.com/agent-framework/agent-skills-for-net-is-now-released, learn.microsoft.com/agent-framework/agents/skills.
...
### K-theory / Grothendieck — researched verdict: DO NOT build a K-theory epic (2026-07-04)

Frugal run wf_e9348f37 (9 confirmed / 16 refuted; academic primaries 403'd, so confirmations rest on convergent independent web searches, not primary sources — treat as *directional*, re-source before betting). The high-value finding is a **negative that saves a rabbit hole**:

- **K-theory proper (K0/K1, vector bundles, C*-algebras) has zero findable published music/audio applications** — the field is empty (medium confidence; existential negatives are never fully provable, but multiple independent searches returned only adjacent *topology*, never K-theory). **Do not scope a "K-theory" epic.**
- **GA's `GrothendieckService` is already the whole story**: it's the Grothendieck *group of a commutative monoid* = the degenerate K0 case = plain group theory. Nothing in the verified literature justifies going beyond it. Keep it; don't "upgrade" it toward higher K-theory.
- **Categorical music theory exists but shows no algorithmic gain**: PK-nets (Popoff/Andreatta/Ehresmann, with the Python `Opycleid` implementation) formalize Klumpenhouwer networks as functors, compatible with GA's DSL grammar — but **no performance benchmark vs baselines was found**. Formalization, not a product lever. Park as "interesting, unproven".
- **The one topological path with measurable results is TDA / persistent homology** (style classification, structural analysis via H0/H1 on simplicial complexes — simplicial homology, NOT K-theory, NOT orbifolds). **And GA already computes β0/β1 via ix-topo** — so the *only* worthwhile "categorical/topological" investment is a **TDA bridge** reusing that existing machinery, not new algebra.
  - **Candidate tracer bullet (if pursued)**: one persistent-homology feature (H1 cycles) over an existing `state/quality/`-style corpus or a voicing-graph, evaluated against a non-topological baseline on a concrete task (style/section classification) — pause rule if it doesn't beat the baseline. Reuses ix-topo. Lower priority than M1-M5.
- Tymoczko's voice-leading orbifold (Science 2006) claims were all refuted 0-3 — **for citation fabrication, not disproof** (the PDF 403'd); its substance is textbook-true but must be re-sourced from a reachable copy before any use. (Another poisoned-cache incident occurred — a ScienceDirect URL held a synthesis memo; cache now gitignored.)
...
### M6 — Adaptive practice engine [product — research-backed, v0.3-validated]

**Verified feasibility** (frugal run wf_b6304907, reframed to reachable GitHub sources — this run *validates the v0.3 workflow*: the same practice question returned zero facts on v0.2 when aimed at academic journals, and solid citable facts on v0.3 when aimed at open-source repos). Scheduling algorithm to reuse: **FSRS (Free Spaced Repetition Scheduler)** — permissive licenses (MIT: py-fsrs, ts-fsrs, rs-fsrs, fsrs4anki; BSD-3: fsrs-rs), **DSR model** (Difficulty/Stability/Retrievability, 21 ML-optimized weights per learner), ports in TS (`ts-fsrs` → React-direct), Python, Rust, Go; .NET consumes it via the fsrs-rs C/FFI binding or a wrapper service. The **fsrs4anki scheduler/optimizer split** (scheduler orders drills, optimizer ML-tunes each learner's difficulty curve on review history) is the directly-transposable pattern for procedural skills. Timing measurement without special hardware: **web-audio-beat-detector (MIT, client-side)**; **aubio (GPL-3.0) and Essentia (AGPL) are copyleft → EXCLUDE**.

- **Tracer bullet**: map one guitar skill (e.g. a scale or chord change) to a DSR card, schedule it with `ts-fsrs` in the frontend, record review outcomes; no ML optimizer yet — just the scheduler over one skill, end to end. Then layer the optimizer once there's review history.
- **Verified honest limits**: three question angles returned NO facts even on reachable sources — Anki v3 scheduler internals (all claims refuted), the SM-2 comparison, and **all competitor pedagogy** (Yousician/Fender Play/Justin Guitar/Simply Guitar — their product pages weren't reachable either). Do not scope competitor-differentiation claims without a separate reachable-source pass.
- **Blocked on**: nothing for the tracer (ts-fsrs is npm-installable). Paths: `ReactComponents/`, a new practice-scheduling service in `Common/GA.Business.*`. **Tribunal: not required** for the tracer. Ties to M5 (audio input can feed review-outcome scoring) once both exist.
...
### Learning Loop — le système apprend de l'usage (idée Swap recadrée, 2026-07-05)

L'idée retenue de Swap (« agentic storefront ») n'est **pas** la monétisation mais le **système qui apprend** : *« learn from your customers — every conversation captures real signal »* → le signal est **owned first-party** et **compose** un meilleur modèle avec le temps, ce qui bat les modèles génériques. Recadré : GA comme **système qui apprend de l'usage**, pas qui vend (décision opérateur 2026-07-05).

**Ce que GA a déjà** (l'essentiel) : les boucles d'apprentissage *côté process* — `/auto-optimize` (Cherny) sur les baselines `state/quality/`, la boucle chatbot-qa, le reliability model G2, le forecast ledger J2, le discovery-engine (generator-verifier), et la doctrine J4 « prepare-the-data » (logger chaque transition action→observation). Plus la thèse « données possédées composent » du Spectral track (synthétiser le volume depuis nos propres grammaires = actif propriétaire).

**Le vrai gap** (le ~20% nouveau) : rien ne capture le signal d'**interaction réelle de l'apprenant** depuis la surface produit (chatbot, practice, voicing-search) comme donnée *owned* qui compose. Les boucles actuelles apprennent du *process de dev*, pas de l'*usage produit* — c'est l'analogue exact de « every conversation captures real signal », version apprentissage.

- **Tracer bullet** : instrumenter UNE interaction (ex. requête chatbot → résolue/non, ou outcome de practice) → log JSONL *owned* → UNE amélioration downstream (re-rank retrieval, ou ajuster la séquence curriculum G1/Seldon), avec baseline avant/après (règle R6). Fin à fin, minuscule, mesurable.
- **Privacy / consent (croise G-guard)** : le signal d'usage EST de la donnée utilisateur → capturer avec consentement, anonymiser, propriété inspectable par l'apprenant. Le modèle de l'humain reste transparent et lui appartient (clause G-guard 1).
- **Réutilise, ne duplique pas** : le schéma de transition J4, les baselines `state/quality/`, nourrit G1/Seldon/M6 (personnalisation) ; re-ranking OPTIC-K possible **sans toucher l'encodeur** (one-way door).
- **Pause rule** : demande de vrais utilisateurs générant du signal ; sans volume/consent, l'epic reste au stade instrumentation (« prepare-the-data », comme J4). **Tribunal : REQUIRED** dès que le signal d'usage influence un modèle servi.
...
- **Tracer bullet** : ingérer les grades existants de `state/quality/pr-grades/` comme observations hari (un belief « fiabilité de X sur tâches de classe Y » par agent), et exposer le score via le MCP hari. Aucune décision automatisée encore — le modèle existe et est honnête, c'est tout. **✅ shipped 2026-07-04** (hari commit c977f88 + ga même jour) : `hari-core reliability <grades-dir>` (`crates/hari-core/src/reliability.rs`) — rapport par agent × classe de tâche (préfixe conventional-commit du titre), lissage pseudo-count épinglé par test (PRIOR_WEIGHT=2, PRIOR_MEAN=0.5), entrée `pooled` = baseline A/B, dégradation honnête (`agent` absent → `unknown`, fichiers non-cartes skippés, n=0 → prior, jamais NaN) ; 8 tests, suite complète verte (196), smoke-testé sur le ledger réel (vide). Côté ga : champ additif `agent` (= auteur de la PR, distinct de `grader`) ajouté à `pr-grade-v1` (SCHEMA.json + README + skill `/grade-last-pr`) — **le ledger est vide aujourd'hui : les scores ne deviennent réels qu'à mesure que `/grade-last-pr` tourne post-merge avec `agent` rempli**. Exposition service **✅ shipped 2026-07-04** (hari commit 3e88010) : requête additive `{"op":"reliability"}` dans le protocole Phase 6 (`hari-core serve`), session-free, `now` injectable, erreurs non-fatales (`invalid_timestamp`/`grades_io`), 198 tests verts, smoke wire vérifié. **Suivi humain** : le binaire `hari-mcp.exe` référencé par les `.mcp.json` (ga + ix) n'a de source dans aucun repo cloné — la committer quelque part avant d'y ajouter le tool `hari_reliability` qui enverra cette requête.
...
- **Tracer bullet** : trois clauses ajoutées au contrat action-boundary (J3, déjà shippé v0.1 DRAFT) + un check : toute lecture du modèle G1 par un acteur autonome est loggée dans l'audit trail existant. Demerzel est owner (persona-requirements : si une persona `giskard` est créée, behavioral test obligatoire dans `tests/behavioral/`). **Design rédigé 2026-07-04** (précondition remplie — G1-ops + G2 existent) : [docs/plans/2026-07-04-arch-giskard-g-guard-design.md](docs/plans/2026-07-04-arch-giskard-g-guard-design.md) — 4 clauses additives (transparence/propriété du modèle humain, non-manipulation, symétrie de fiabilité, audit de toute lecture de mind-model) + le seul check mécanique (read-audit dans l'aggregateur action-boundary). Ship gated tribunal.
