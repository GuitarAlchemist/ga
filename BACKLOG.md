...
- **Validation externe (2026-07-05)** : Swap Commerce (« agentic storefront ») vend exactement ce pari côté e-commerce — *« Generic AI models learn broadly. Agentic storefronts learn from your customers. Every conversation captures real shopping signals. »* C'est le pattern **intention captée en conversation → first-party owned → composée en personnalisation**, qui bat les modèles génériques. GA l'applique à l'*apprenant* (pas à l'acheteur) via G1 + Seldon + M6. Retenu comme **signal de marché confirmant la direction** (owned-intent compounds), pas comme brique à adopter ni comme piste commerce — la monétisation reste hors périmètre tant qu'une offre n'est pas définie (décision opérateur 2026-07-05 : note de cross-ref uniquement, pas d'epic commerce).
