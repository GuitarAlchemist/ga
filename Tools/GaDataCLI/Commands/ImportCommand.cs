...
{
    public static async Task<int> ExecuteAsync(string? dataFile, string connectionString, string databaseName, string collectionName, bool force = false)
    {
        dataFile ??= @"C:\Temp\GaExport\all-chords.json";
        ...
        try
        {
            ...
            if (!File.Exists(dataFile))
            {
                ...
                AnsiConsole.MarkupLine("  GaDataCLI --export chords -o C:\\Temp\\GaExport -q");
                return 1;
            }
            ...
            if (existingCount > 0)
            {
                ...
                if (!force && !AnsiConsole.Confirm("Do you want to drop the collection and reimport?"))
                {
